...
$stmt = $conn->prepare("UPDATE recipes SET name=?, description=?, category=?, mood=?, prep_time=?, servings=?, instructions=?, ingredients=?, image=? WHERE id=? AND user_id=?");
$stmt->bind_param("ssssiisssi i", $name, $description, $category, $mood, $prep_time, $servings, $instructions, $ingredients, $image, $id, $user_id);
